...
import {
  ...
  DialogTitle,
  DialogDescription,
} from "./ui/dialog"; // Importa el Dialog de shadcn
import { Badge } from './ui/badge';
import { ScrollArea } from './ui/scroll-area'; // Necesitarás 'scroll-area'
...
export function HistorialVentaDialog({ ventaId, onOpenChange }: Props) {
    ...
    useEffect(() => {
        // Asegurarse de que el scroll esté deshabilitado en el body
        ...
        // Función de limpieza para re-habilitar el scroll
        return () => {
                            ...
                            <div key={h.id} className="p-4 border rounded-lg">
                                <div className="flex justify-between items-center mb-2 flex-wrap gap-2">
                                    ...
                                    <span className="text-xs text-muted-foreground">
                                        {new Date(h.fecha).toLocaleString()} por {h.usuario?.nombre || 'N/A'}
                                    </span>
                                </div>
